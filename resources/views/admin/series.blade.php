...
<div class="container mx-auto px-4 py-8">
                ...
                <div class="bg-dark rounded-lg overflow-hidden border border-gray-800 series-card relative" data-title="{{ strtolower($series->title) }}">
                    <div class="relative">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-4">
                            <h3 class="text-white font-bold">{{ $series->title }}</h3>
                            <div class="flex items-center mt-1">
                                <span class="text-yellow-500 mr-1"><i class="ri-star-fill"></i></span>
                                <span class="text-white text-sm">{{ $series->rating ?? 'N/A' }}</span>
                                @if($series->year)
                                <span class="text-gray-400 text-sm ml-2">{{ $series->year }}</span>
                                @endif
                            </div>
                        </div>
                        @if($series->is_banned)
                    ...
                    <div class="p-4">
                        <div class="mb-3">
                            <p class="text-gray-300 text-sm line-clamp-3 h-18">{{ $series->description ?? 'No description available.' }}</p>
                        </div>
                        @if($series->categories->isNotEmpty())
                        <div class="mb-2">
                            <span class="text-gray-400 text-xs">Categories:</span>
                            <span class="text-gray-300 text-xs">{{ $series->categories->pluck('name')->implode(', ') }}</span>
                        </div>
                        @endif
                        @if($series->seasons)
                        <div class="mb-2">
                            <span class="text-gray-400 text-xs">Seasons:</span>
                            <span class="text-gray-300 text-xs">{{ $series->seasons }}</span>
                            @if($series->episodes_per_season)
                            <span class="text-gray-400 text-xs ml-2">Eps/Season:</span>
                            <span class="text-gray-300 text-xs">{{ $series->episodes_per_season }}</span>
                            @endif
                        </div>
                        @endif
                        <div class="mb-2">
                            <span class="text-gray-400 text-xs">Created:</span>
                            <span class="text-gray-300 text-xs">{{ $series->created_at->format('M d, Y') }}</span>
                        </div>
                        <div class="mt-4 flex justify-between items-center">
                            <div>
                                <button 
                                    class="px-3 py-1 rounded text-sm {{ $series->is_banned ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600' }} text-white ban-button"
                                    data-id="{{ $series->id }}"
                                    data-is-tmdb="false"
                                    onclick="toggleBan({{ $series->id }}, false)"
                                >
                                    {{ $series->is_banned ? 'Unban' : 'Ban' }}
                                </button>
                            </div>
                            <div>
                                <button 
                                    onclick="editTvShow({{ $series->id }})" 
                                    class="text-blue-500 hover:text-blue-700 mr-2"
                                >
                                    <i class="ri-edit-line"></i>
                                </button>
                                <button 
                                    onclick="deleteTvShow({{ $series->id }})" 
    ...
    <div id="api-content" class="tab-content {{ request('tab') == 'api' ? '' : 'hidden' }}">
                ...
                <div class="bg-dark rounded-lg overflow-hidden border border-gray-800 series-card relative" data-title="{{ strtolower($series['name'] ?? 'unknown title') }}">
                    <div class="relative">
                        <img 
                            ...
                            alt="{{ $series['name'] ?? 'TV Show' }}"
                            class="w-full h-64 object-cover card-image transition-all duration-300"
                        >
                        <div class="absolute inset-0 bg-gradient-to-t from-black to-transparent opacity-70"></div>
                        <div class="absolute bottom-0 left-0 right-0 p-4">
                            <h3 class="text-white font-bold">{{ $series['name'] ?? 'Unknown Title' }}</h3>
                            <div class="flex items-center mt-1">
                                <span class="text-yellow-500 mr-1"><i class="ri-star-fill"></i></span>
                                <span class="text-white text-sm">{{ $series['vote_average'] ? number_format($series['vote_average'], 1) : 'N/A' }}</span>
                                @if($series['first_air_date'])
                                <span class="text-gray-400 text-sm ml-2">{{ substr($series['first_air_date'], 0, 4) }}</span>
                                @endif
                            </div>
                        </div>
                        @if(in_array($series['id'], $bannedApiSeriesIds ?? []))
                        <div class="absolute top-2 right-2 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded">
                            BANNED
                        </div>
                        @endif
                    ...
                    <div class="p-4">
                        <p class="text-gray-300 text-sm line-clamp-3 h-18">{{ $series['overview'] ?? 'No description available.' }}</p>
                        <div class="mt-4 flex justify-between items-center">
                            <button 
                                class="px-3 py-1 rounded text-sm {{ in_array($series['id'], $bannedApiSeriesIds ?? []) ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600' }} text-white ban-button"
                                data-id="{{ $series['id'] }}"
                                data-is-tmdb="true"
                                onclick="toggleBan({{ $series['id'] }}, true)"
                            >
                                {{ in_array($series['id'], $bannedApiSeriesIds ?? []) ? 'Unban' : 'Ban' }}
                            </button>
                        </div>
                    </div>
                </div>
        ...
        @if($apiSeriesPaginator && $apiSeriesPaginator->lastPage() > 1)
        <div class="mt-8 flex justify-center">
            <nav class="flex items-center">
                @if($apiSeriesPaginator->onFirstPage())
                    <span class="px-3 py-1 bg-gray-800 text-gray-500 rounded-l cursor-not-allowed">Previous</span>
                @else
                    <a href="{{ $apiSeriesPaginator->previousPageUrl() }}" class="px-3 py-1 bg-gray-800 text-primary hover:bg-gray-700 rounded-l">Previous</a>
                @endif
                ...
                <div class="flex">
                    ...
                    @if($start > 1)
                        <a href="{{ $apiSeriesPaginator->url(1) }}" class="px-3 py-1 bg-gray-800 text-white hover:bg-gray-700">1</a>
                        @if($start > 2)
                    ...
                    @for($i = $start; $i <= $end; $i++)
                        <a href="{{ $apiSeriesPaginator->url($i) }}" 
                           class="px-3 py-1 {{ $apiSeriesPaginator->currentPage() == $i ? 'bg-primary text-white' : 'bg-gray-800 text-white hover:bg-gray-700' }}">
                            {{ $i }}
                        </a>
                    @endfor
                    
                    @if($end < $apiSeriesPaginator->lastPage())
                        @if($end < $apiSeriesPaginator->lastPage() - 1)
                            <span class="px-2 py-1 bg-gray-800 text-gray-500">...</span>
                        @endif
                        <a href="{{ $apiSeriesPaginator->url($apiSeriesPaginator->lastPage()) }}" class="px-3 py-1 bg-gray-800 text-white hover:bg-gray-700">{{ $apiSeriesPaginator->lastPage() }}</a>
                    @endif
                </div>
                
                @if($apiSeriesPaginator->hasMorePages())
                    <a href="{{ $apiSeriesPaginator->nextPageUrl() }}" class="px-3 py-1 bg-gray-800 text-primary hover:bg-gray-700 rounded-r">Next</a>
                @else
    ...
    <div id="tv-show-modal" class="fixed inset-0 bg-black bg-opacity-75 hidden items-center justify-center z-50 overflow-y-auto">
        <div class="bg-dark rounded-lg w-full max-w-2xl p-6 m-4 relative">
            ...
            <form id="tv-show-form" action="{{ route('admin.series.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @csrf
