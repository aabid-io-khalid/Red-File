        ...
        <div class="container mx-auto">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                <h1 class="text-3xl font-bold mb-4 md:mb-0">
                    ...
                    @elseif($sort == 'trending') Trending Movies
                    @elseif($sort == 'release_date' && $order == 'desc') New Releases
                    @elseif($sort == 'vote_average' && $order == 'desc') Top Rated Movies
                    @else Browse Movies @endif
                </h1>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
                @forelse ($content as $item)
                    ...
                    @endphp
                    @if($id && is_string($title) && is_string($posterPath))
                        <a href="{{ $isLocal ? route('movies.local', $id) : route('movies.show', $id) }}"
